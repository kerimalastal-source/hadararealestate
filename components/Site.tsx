...
export function Site({ locale }: { locale: Locale }) {
  ...
  return <>
    ...
    <section className="partners section wrap" id="partners"><div className="section-heading"><div><p className="kicker dark">{d.partnersKicker}</p><h2>{d.partnersTitle}</h2></div><span className="line"/></div><div className="partner-grid">{d.partners.map(p => <div className="partner-logo" key={p.name}><Image src={p.image} alt={p.name} width={p.width} height={p.height}/></div>)}</div></section>
