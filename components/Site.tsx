...
export function Site({ locale }: { locale: Locale }) {
  const d = copy[locale];
  return <>
    <section className="hero" id="home"><Image src="/images/istanbul.jpg" alt="Istanbul and the Bosphorus" fill priority sizes="100vw" /><div className="hero-overlay"/><div className="wrap hero-copy"><p className="kicker">{d.heroKicker}</p><h1>{d.heroTitle}</h1><p className="hero-text">{d.heroText}</p><a className="button light" href="#projects">{d.explore}<Arrow/></a></div><p className="since">{d.since}</p></section>
    ...
    <section className="invest section wrap" id="invest"><div className="invest-image"><Image src="/images/interior.jpg" alt="Refined residential interior" fill sizes="(max-width:800px) 100vw, 50vw"/></div><div><p className="kicker dark">{d.investKicker}</p><h2>{d.investTitle}</h2><p>{d.investText}</p><ul>{d.investPoints.map(x => <li key={x}>✓ {x}</li>)}</ul></div></section>
    <section className="contact section" id="contact"><div className="wrap contact-grid"><div><p className="kicker">HADARA · ISTANBUL</p><h2>{d.ctaTitle}</h2><p>{d.ctaText}</p></div><form className="form" onSubmit={undefined}><div>{d.form.map((x,i) => <label key={x}>{x}<input type={i===2?'email':i===3?'tel':'text'} name={`field-${i}`}/></label>)}</div><button type="button">{d.send}<Arrow/></button></form></div></section>
  </>;
}
