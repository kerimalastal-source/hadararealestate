...
export function Site({ locale }: { locale: Locale }) {
  const d = copy[locale];
  return <>
    <section className="hero" id="home"><Image src="/images/istanbul.jpg" alt={d.heroImageAlt} fill priority sizes="100vw" /><div className="hero-overlay"/><div className="wrap hero-copy"><p className="kicker">{d.heroKicker}</p><h1>{d.heroTitle}</h1><p className="hero-text">{d.heroText}</p><Link className="button light" href={`/${locale}/projects`}>{d.explore}<Arrow/></Link></div><p className="since">{d.since}</p></section>
    ...
    <section className="projects section" id="projects"><div className="wrap section-heading"><div><p className="kicker dark">{d.projectsKicker}</p><h2>{d.projectsTitle}</h2></div><span className="line"/></div><div className="wrap project-list">{d.projects.map((p,i) => <article className="project" key={p.name}><div className="project-image"><Image src={p.image} alt={p.name} fill sizes="(max-width:800px) 100vw, 50vw"/></div><div className="project-copy"><p className="project-tag">{p.tag}</p><h3>{p.name}</h3><p>{p.description}</p><ul>{p.facts.map(x => <li key={x}>{x}</li>)}</ul><Link className="text-link" href={`/${locale}/projects/${p.slug}`}>{d.viewProject}<Arrow/></Link></div></article>)}</div></section>
